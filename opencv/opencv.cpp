...
void onChange(int pos)
{
	...
	image2 = cvLoadImage("i.jpg", 1); //显示图片
	image = cvLoadImage("i.jpg", 0);
...
/*
// opencv.cpp : ¶¨Òå¿ØÖÆÌ¨Ó¦ÓÃ³ÌÐòµÄÈë¿Úµã¡£
//
#include <iostream>
#include <fstream>
#include <algorithm>
...
int main(int argc, char* argv[])
{
	...
	cv::Mat src;
	cv::VideoCapture capture("h1.avi");
	...
	{
		//capture >> src;
		src=cv::imread("sample_output.jpg");
		if (src.empty()) break;
...
Mat contour(Mat& image)
{
	...
	//drawContours(image, contours, -1, Scalar(255, 0, 255),-1);
	for (int j = 0; j < contours.size(); j++)
		//	for (int i = 0; i < contours[j].size(); i++)
	{
		/*vector<Point> hull;
		convexHull(Mat(contours[j]), hull);
		vector<Point>::const_iterator ith = hull.begin();
		while (ith != (hull.end() - 1))
		{
			line(image, *ith, *(ith + 1), Scalar(255), 2);
			++ith;
		}
		line(image, *ith, *(hull.begin()), Scalar(255), 2);*/
		// 轮廓表示为一个多边形 
		vector<Point> poly;
